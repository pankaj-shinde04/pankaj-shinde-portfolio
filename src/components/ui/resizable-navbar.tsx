    ...
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-5 py-2 text-neutral-600 dark:text-neutral-300"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
...
export const NavbarLogo = ({
  ...
  return (
    <a
      href={href || "#"}
      className="relative z-20 mr-8 flex shrink-0 items-center space-x-2 px-2 py-1 text-sm font-normal text-black whitespace-nowrap"
    >
      <span className="font-bold text-base text-black dark:text-white tracking-tight whitespace-nowrap">
        {name || "Portfolio"}
      </span>
    </a>
  );
...
export const NavbarButton = ({
  ...
  const variantStyles = {
    ...
    secondary: "bg-transparent shadow-none text-neutral-600 dark:text-white hover:text-black",
